# nwm-work

```json
[
  {
  
    "name": "any-rule",
    "publisher": "russell",
    "version": "0.3.9"
  },
  {
   
    "name": "at-helper",
    "publisher": "ZihanLi",
    "version": "0.0.4"
  },
  {
  
    "name": "auto-close-tag",
    "publisher": "formulahendry",
    "version": "0.5.10"
  },
  {
   
    "name": "auto-rename-tag",
    "publisher": "formulahendry",
    "version": "0.1.6"
  },
  {
   
    "name": "beautify",
    "publisher": "HookyQR",
    "version": "1.5.0"
  },
  {
   
    "name": "code-runner",
    "publisher": "formulahendry",
    "version": "0.11.2"
  },
  {
   
    "name": "code-settings-sync",
    "publisher": "Shan",
    "version": "3.4.3"
  },
  {
   
    "name": "cssrem",
    "publisher": "cipchk",
    "version": "2.2.0"
  },
  {
   
    "name": "debugger-for-chrome",
    "publisher": "msjsdiag",
    "version": "4.12.12"
  },
  {
   
    "name": "easy-less",
    "publisher": "mrcrowl",
    "version": "1.7.2"
  },
  {
   
    "name": "easysass",
    "publisher": "spook",
    "version": "0.0.6"
  },
  {
   
    "name": "EditorConfig",
    "publisher": "EditorConfig",
    "version": "0.16.4"
  },
  {
   
    "name": "element-ui-snippets",
    "publisher": "SS",
    "version": "0.7.2"
  },
  {
   
    "name": "html-css-class-completion",
    "publisher": "Zignd",
    "version": "1.19.0"
  },
  {
    
    "name": "html-snippets",
    "publisher": "abusaidm",
    "version": "0.2.1"
  },
  {
    
    "name": "indenticator",
    "publisher": "SirTori",
    "version": "0.7.0"
  },
  {
   
    "name": "LiveServer",
    "publisher": "ritwickdey",
    "version": "5.6.1"
  },
  {
  
    "name": "markdown-preview-enhanced",
    "publisher": "shd101wyy",
    "version": "0.5.18"
  },
  {
   
    "name": "minapp-vscode",
    "publisher": "qiu8310",
    "version": "2.2.2"
  },
  {
    
    "name": "miniapp-helper",
    "publisher": "overtrue",
    "version": "1.0.3"
  },
  {
   
    "name": "one-monokai",
    "publisher": "azemoh",
    "version": "0.5.0"
  },
  {
    
    "name": "open-in-browser",
    "publisher": "techer",
    "version": "2.0.0"
  },
  {
    "publisher": "ionutvmi",
    "version": "1.17.1"
  },
  {
    
    "name": "path-intellisense",
    "publisher": "christian-kohler",
    "version": "2.3.0"
  },
  {
    
    "name": "pdf",
    "publisher": "tomoki1207",
    "version": "1.1.0"
  },
  {
   
    "name": "php-docblocker",
    "publisher": "neilbrayfield",
    "version": "2.1.0"
  },
  {
   
    "name": "php-intellisense",
    "publisher": "felixfbecker",
    "version": "2.3.14"
  },
  {
    
    "name": "phpstorm-snippets",
    "publisher": "phiter",
    "version": "1.1.2"
  },
  {
    
    "name": "prettier-vscode",
    "publisher": "esbenp",
    "version": "5.8.0"
  },
  {
   
    "name": "px-to-vw",
    "publisher": "leechaojiang",
    "version": "1.0.2"
  },
  {
    
    "name": "ReactSnippets",
    "publisher": "xabikos",
    "version": "2.4.0"
  },
  {
    
    "name": "remote-wsl",
    "publisher": "ms-vscode-remote",
    "version": "0.51.4"
  },
  {
  
    "name": "stylus",
    "publisher": "Alan",
    "version": "0.0.4"
  },
  {
   
    "name": "tslint",
    "publisher": "eg2",
    "version": "1.0.44"
  },
  {
   
    "name": "vant-snippets",
    "publisher": "fishku",
    "version": "0.1.5"
  },
  {
   
    "name": "vetur",
    "publisher": "octref",
    "version": "0.31.3"
  },
  {
   
    "name": "vscode-echarts-extension",
    "publisher": "susiwen8",
    "version": "0.2.3"
  },
  {
   
    "name": "vscode-element-helper",
    "publisher": "ElemeFE",
    "version": "0.5.6"
  },
  {
   
    "name": "vscode-eslint",
    "publisher": "dbaeumer",
    "version": "2.1.13"
  },
  {
   
    "name": "vscode-file-peek",
    "publisher": "abierbaum",
    "version": "1.0.1"
  },
  {
   
    "name": "vscode-gutter-preview",
    "publisher": "kisstkondoros",
    "version": "0.26.2"
  },
  {
    
    "name": "vscode-html-css",
    "publisher": "ecmel",
    "version": "1.2.2"
  },
  {
   
    "name": "vscode-import-cost",
    "publisher": "wix",
    "version": "2.13.0"
  },
  {
    
    "name": "vscode-language-pack-zh-hans",
    "publisher": "MS-CEINTL",
    "version": "1.51.2"
  },
  {
   
    "name": "vscode-markdownlint",
    "publisher": "DavidAnson",
    "version": "0.40.3"
  },
  {
    
    "name": "vscode-npm-script",
    "publisher": "eg2",
    "version": "0.3.21"
  },
  {
   
    "name": "vscode-placeholder-images",
    "publisher": "JakeWilson",
    "version": "0.1.0"
  },
  {
   
    "name": "vscode-stylelint",
    "publisher": "stylelint",
    "version": "0.86.0"
  },
  {
   
    "name": "vue-definition",
    "publisher": "wangcunji",
    "version": "1.0.8"
  },
  {
    
    "name": "vue-peek",
    "publisher": "dariofuzinato",
    "version": "1.0.2"
  },
  {
   
    "name": "vue-vscode-snippets",
    "publisher": "sdras",
    "version": "2.2.0"
  }
]
```
e431(21-5-2)
```json
Extensions Added:
  any-rule v0.3.9
  at-helper v0.0.4
  auto-close-tag v0.5.10
  auto-rename-tag v0.1.6
  beautify v1.5.0
  code-runner v0.11.3
  code-settings-sync v3.4.3
  cssrem v2.2.0
  debugger-for-chrome v4.12.12
  docthis v0.8.2
  easy-less v1.7.2
  EditorConfig v0.16.4
  html-css-class-completion v1.20.0
  html-snippets v0.2.1
  indenticator v0.7.0
  LiveServer v5.6.1
  markdown-preview-enhanced v0.5.18
  minapp-vscode v2.2.2
  miniapp-helper v1.0.3
  open-in-browser v2.0.0
  path-autocomplete v1.17.1
  path-intellisense v2.3.0
  pdf v1.1.0
  php-docblocker v2.1.0
  php-intellisense v2.3.14
  phpstorm-snippets v1.1.2
  px-to-vw v1.0.2
  ReactSnippets v2.4.0
  remote-wsl v0.54.6
  stylus v0.0.4
  tslint v1.0.46
  vetur v0.33.1
  vscode-eslint v2.1.20
  vscode-file-peek v1.0.1
  vscode-gutter-preview v0.27.1
  vscode-html-css v1.10.2
  vscode-icons v11.4.0
  vscode-import-cost v2.15.0
  vscode-language-pack-zh-hans v1.55.2
  vscode-markdownlint v0.40.4
  vscode-npm-script v0.3.21
  vscode-placeholder-images v0.1.0
  vue-definition v1.0.8
  vue-peek v1.0.2
  vue-vscode-snippets v2.2.0
  webpack v2.2.0
  webpack-4-snippets v0.0.3
  webpack-snippets v0.0.4
```
